...
btnHistoryEl.addEventListener('click', function(){
    ...
    diceHistory.forEach((entry, index) => {// κάθε νέα είσοδος (entry) μπαίνει στη λίστα του ιστορικού (diceHistory)
        ...
        listItem.innerHTML = `Roll ${index + 1}:\u00A0<span style="color: blue;">${entry.dice1}</span>\u00A0\u00A0<span style="color: blue;">${entry.dice2}</span> \u00A0<span style="color: blue;">${entry.test}</span>\u00A0\u00A0Sum: <span style="color: blue;">${entry.dice1+entry.dice2}</span>`;
